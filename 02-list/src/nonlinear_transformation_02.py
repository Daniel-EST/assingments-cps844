...
class LinearModel():
    ...
    def predict(self, inputs: List[float]) -> int:
        ...
        - 1 if the point is above the target function, -1 otherwise..
        """
        wtx = np.matmul((1, inputs[0], inputs[1], inputs[0] *
                        inputs[1], inputs[0]**2, inputs[1]**2), self.__weights)
    ...
    def train(self, inputs: List[Tuple[float]], outputs: List[int]) -> None:
        ...
        """
        x = [(1, x[0], x[1], x[0]*x[1], x[0]**2, x[1]**2) for x in inputs]
...
def run_experiments():
    ...
    experiment.start()
    w = list(experiment.linear_model.weights)
    print(
        "g(x1, x2)=sign({:.2f} + {:.2f} + {:.2f}x2 + {:.2f}x1x2 + {:.2f}x1^2 + {:.2f}x2^2)".format(*w)
    )
    print(
