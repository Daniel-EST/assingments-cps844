...
class Perceptron():
    ...
    def train(self, inputs: List[Tuple[float]], outputs: List[int], max_iterations: int = 100) -> Tuple[int, List[float]]:
        """
...
def run_experiments():
    ...
    plt.savefig("N100_10_lm_steps.png")
    # plt.show()
    plt.clf()
    ...
    plt.plot(list(range(len(experiment.pocket_errors))),
             experiment.pocket_errors, 'k--', label="f(x)")
    plt.savefig("N100_50_lm_steps.png")
    # plt.show()
    plt.clf()
